...
template_file=".github/github-actions-test.template.rb"

output_file=".github/github-actions-test.rb"
...
get_url_and_sha256() {
    ...
    local url="https://github.com/noborus/github-actions-test/releases/download/$VERSION/$file_name"
    local sha256=$(sha256sum "$file_path" | awk '{print $1}')
    echo "$url $sha256"
}

darwin_arm64=$(get_url_and_sha256 "dist/github-actions-test_${VERSION}_darwin_arm64.zip")
darwin_arm64_url=$(echo "${darwin_arm64% *}" | sed 's/\//\\\//g')
darwin_arm64_sha256=${darwin_arm64#* }

darwin_amd64=$(get_url_and_sha256 "dist/github-actions-test_${VERSION}_darwin_amd64.zip")
darwin_amd64_url=$(echo "${darwin_amd64% *}" | sed 's/\//\\\//g')
darwin_amd64_sha256=${darwin_amd64#* }

linux_arm64=$(get_url_and_sha256 "dist/github-actions-test_${VERSION}_linux_arm64.zip")
linux_arm64_url=$(echo "${linux_arm64% *}" | sed 's/\//\\\//g')
linux_arm64_sha256=${linux_arm64#* }

linux_amd64=$(get_url_and_sha256 "dist/github-actions-test_${VERSION}_linux_amd64.zip")
